...
- Checkout page file: `dfy-adult-checkout.html` — URL: `/dfy-adult-checkout-page/` — houses the ThriveCart embed (account `adoptionssimplified`, product `8`, embed token `tc-adoptionssimplified-8-C2K6LW`)
...
| Date | Change | Author |
|---|---|---|
| 2026-09-18 | v1.22 — **Third ThriveCart token regen for product 8 today — sixth overall.** Another round of ThriveCart admin changes by Tyler regenerated the token again. Product ID, account, and script src unchanged. **Old to new token:** `E122QL` to `C2K6LW`. Swapped in `dfy-adult-checkout.html` (2 places) and in the one current-spec reference in this doc. Historical change-log entries left intact. | Claude / Tyler |
...
| 2026-05-12 | v1.17 — ThriveCart embed token regenerated for product 8 (DFY adult). Product ID unchanged. Old token: `POHMYG` → new token: `FGQ5PI`. Token swapped in `dfy-adult-checkout.html` and across all token references in this spec doc via `replace_all`. Part of a four-product regen batch this same day — Tyler made admin-side adjustments to all four ThriveCart products at once and all four products (4 / 7 / 8 / 10) received new tokens together. | Claude / Tyler |
| 2026-05-12 | v1.18 — ThriveCart embed token regenerated **again** for product 8 (DFY adult) after Tyler made an additional admin-side tweak later the same day. Product ID unchanged. Old token: `FGQ5PI` → new token: `OBAALE`. Token swapped in `dfy-adult-checkout.html` and across all token references in this spec doc via `replace_all`. Only product 8 affected — the other three products (4 / 7 / 10) keep the tokens set earlier today. | Claude / Tyler |
| 2026-09-18 | v1.20 — **ThriveCart token regen for product 8 (DFY adult) — fourth regen overall for this product.** Triggered by Tyler setting the post-purchase success redirect to the new `/thank-you/` page in the ThriveCart admin; saving the product regenerated the embeddable token, the same behavior seen after every prior admin-side change on this account. Product ID unchanged (`8`), account unchanged (`adoptionssimplified`), script src unchanged. **Old → new token:** `1PK8KJ` → `DGBV8Y`. Swapped in `dfy-adult-checkout.html` (2 places — `data-thrivecart-embeddable` attr on the div + `id` on the script) and in this doc's one current-spec reference (the Section 8 checkout-page file/URL/token line). Historical change-log entries naming the older tokens were deliberately left unchanged so the regen history stays accurate. | Claude / Tyler |
| 2026-05-12 | v1.19 — Two changes in one batch. **(1) Token regen:** product 8 (DFY adult) third regen overall. Product ID unchanged. Old token: `OBAALE` → new token: `1PK8KJ`. Token swapped in `dfy-adult-checkout.html` and across all token references in this spec doc via `replace_all`. **(2) Purchase-agreement line added** to `dfy-adult-checkout.html` immediately above the existing reassurance strip: *"By purchasing you agree to our refund policy."* with "refund policy" linked to `https://adoptionformsexpress.com/refund-policy/`. The link opens in a new tab (`target="_blank" rel="noopener"`) so it doesn't pull buyers away from completing their ThriveCart form. Reuses the existing `.refund-note` CSS class for visual consistency with the "Wrong package?" line below the strip. Same line was added uniformly to all four checkout HTML files in this same batch. | Claude / Tyler |
