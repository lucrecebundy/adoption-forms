...
```html
<div class="tc-v2-embeddable-target" data-thrivecart-account="adoptionssimplified" data-thrivecart-tpl="v2" data-thrivecart-product="10" data-thrivecart-embeddable="tc-adoptionssimplified-10-5TNCQG"></div>
<script async src="//tinder.thrivecart.com/embed/v2/thrivecart.js" id="tc-adoptionssimplified-10-5TNCQG"></script>
```

Account `adoptionssimplified` · Product ID `10` · Embed token `tc-adoptionssimplified-10-5TNCQG`. Paste at both CTA locations (Section 4.2 hero + Section 4.7 final CTA) during HTML build.
...
| 2 | Final support email for the page | ✅ Resolved 2026-05-12 | Locked: `info@adoptionformsexpress.com`. Single canonical address used across every HTML page (nav, escape hatch, footer) and in every spec doc that surfaces a contact. Replaces the prior `adoptionformsexpress@gmail.com` Gmail and the recommended-but-never-deployed `support@…` placeholder. |
...
| 8 | ThriveCart embed snippet | ✅ Resolved 2026-04-30 | Production snippet captured verbatim in Section 8. Account: `adoptionssimplified` · Product `10` · Embed token `tc-adoptionssimplified-10-5TNCQG`. To be pasted at both CTA locations during HTML build. |
...
| Date | Change | Author |
|---|---|---|
| 2026-09-18 | v2.22 — **Fourth ThriveCart token regen for product 10 today — eleventh overall.** Another round of ThriveCart admin changes by Tyler regenerated the token again. Product ID, account, and script src unchanged. **Old to new token:** `UCMYFZ` to `5TNCQG`. Swapped in `dfy-minor-child-checkout.html` (2 places) and in the five current-spec references in this doc. Historical change-log entries naming older tokens left intact. | Claude / Tyler |
| 2026-09-18 | v2.21 — **Third ThriveCart token regen for product 10 today — tenth overall.** Tyler corrected the post-purchase setup in ThriveCart: the `/thank-you/` URL had been entered as a product-delivery link (which renders a "click here to access" link on ThriveCart hosted confirmation page) rather than as an automatic success redirect. Fixing that regenerated the token again. Product ID, account, and script src unchanged. **Old to new token:** `X801EE` to `UCMYFZ`. Swapped in `dfy-minor-child-checkout.html` (2 places) and in the five current-spec references in this doc. Historical change-log entries naming older tokens left intact. | Claude / Tyler |
...
| 2026-05-11 | v2.14 — ThriveCart embed token regenerated again for product 10 after Tyler made another adjustment in the ThriveCart admin. Same product, fourth token regen overall. Product ID unchanged. Old token: HV1ZXD → new token: W3F688. Token swapped in `dfy-minor-child-checkout.html` (lines 397–398) and across all token references in this spec doc via `replace_all`. Other three checkout files (DFY adult / DIY minor / DIY adult) unaffected. | Claude / Tyler |
| 2026-05-12 | v2.15 — Contact email standardized site-wide. **Open Question #2** (final support email for the page) ✅ resolved with `info@adoptionformsexpress.com` (chosen over the originally recommended `support@…` because a single inbox is simpler to operate at this stage). `dfy-minor-child.html` + `dfy-minor-child-checkout.html` updated to swap `adoptionformsexpress@gmail.com` → `info@adoptionformsexpress.com`. Part of the same site-wide standardization that resolves master Open Decision #18. | Claude / Tyler |
| 2026-05-12 | v2.16 — ThriveCart embed token regenerated for product 10 (DFY minor child) — fifth regen overall for this product. Product ID unchanged. Old token: `W3F688` → new token: `JB96LS`. Token swapped in `dfy-minor-child-checkout.html` and across all token references in this spec doc via `replace_all`. Part of a four-product regen batch this same day — Tyler made admin-side adjustments to all four ThriveCart products at once and all four products (4 / 7 / 8 / 10) received new tokens together. | Claude / Tyler |
| 2026-05-12 | v2.17 — ThriveCart embed token regenerated **again** for product 10 (DFY minor child) after Tyler made additional minor admin-side adjustments. Sixth regen overall for this product. Product ID unchanged. Old token: `JB96LS` → new token: `TAZ7KX`. Token swapped in `dfy-minor-child-checkout.html` and across all token references in this spec doc via `replace_all`. Only product 10 affected this round. | Claude / Tyler |
| 2026-05-12 | v2.18 — Two changes in one batch. **(1) Token regen:** product 10 (DFY minor child) seventh regen overall. Product ID unchanged. Old token: `TAZ7KX` → new token: `LB4DW4`. Token swapped in `dfy-minor-child-checkout.html` and across all token references in this spec doc via `replace_all`. **(2) Purchase-agreement line added** to `dfy-minor-child-checkout.html` immediately above the existing reassurance strip: *"By purchasing you agree to our refund policy."* with "refund policy" linked to `https://adoptionformsexpress.com/refund-policy/`. The link opens in a new tab (`target="_blank" rel="noopener"`) so it doesn't pull buyers away from completing their ThriveCart form. Reuses the existing `.refund-note` CSS class for visual consistency with the "Wrong package?" line below the strip. Same line was added uniformly to all four checkout HTML files in this same batch. Visual order on the checkout page is now: embed → "By purchasing..." (agreement) → reassurance strip → "Wrong package?..." (existing refund-note). | Claude / Tyler |
