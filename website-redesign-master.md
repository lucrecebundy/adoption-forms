...
| 22 | Future task: dedicated refund-policy page | Open | Build a dedicated refund-policy page detailing the full policy: 7-day reply window, delivery-gated refund eligibility, no-refunds-after-delivery, the buyer's-confirmation-email step. **Surface decision (locked 2026-05-05):** the page is NOT linked from the site footer. Tyler's preference is to surface the link in the post-purchase invoice/receipt only — visitors see the policy at the moment it actually matters (after they've paid) rather than burying it in the footer of marketing pages. Until the page exists, the policy is summarized in the FAQ on each sales page (Q6 — same Q&A across all four) and in the line on each checkout page. Hosting location (own site `/refund-policy/` vs. ThriveCart-hosted page vs. PDF) is open. |
...
|---|---|---|---|
| 2026-05-05 | All four checkout HTML files + their spec docs, **Master** | ThriveCart logo updated, which regenerated the embeddable tokens for all four products. Updated all four checkout HTML files with new tokens (DFY minor 10: TOD8DE → UAXWPD; DFY adult 8: 0FGCA9 → POHMYG; DIY minor 4: TTC0UE → KP063D; DIY adult 7: A4PSQA → F1ED0D). Product IDs unchanged. All four spec docs updated to reflect the new snippets. Each spec doc bumped by 0.1. **Note:** product 10 (DFY minor) has since been regenerated three more times — current product 10 token is **W3F688** (see 2026-05-11 entry below). The tokens for products 4, 7, and 8 above are still the live current tokens. | Claude / Tyler |
...
|---|---|---|---|
| **Master** | `website-redesign-master.md` | v1.17 | 🟡 Active |
...
| DFY minor child (sales + checkout) | `dfy-minor-child-page-v1.md` | v2.14 | 🔵 Built — `dfy-minor-child.html` (sales page) + `dfy-minor-child-checkout.html` (transactional checkout, ThriveCart product 10) both on disk, ready for WordPress deploy |
| DFY sales — adult | `dfy-adult-page-v1.md` | v1.16 | 🔵 Built — `dfy-adult.html` (sales page) + `dfy-adult-checkout.html` (transactional checkout, ThriveCart product 8) both on disk, ready for WordPress deploy |
| DIY minor child (sales + checkout) | `diy-minor-child-page-v1.md` | v1.10 | 🔵 Built — `diy-minor-child.html` (sales page) + `diy-minor-child-checkout.html` (transactional checkout, ThriveCart product 4) both on disk, ready for WordPress deploy |
| DIY adult (sales + checkout) | `diy-adult-page-v1.md` | v1.7 | 🔵 Built — `diy-adult.html` (sales page) + `diy-adult-checkout.html` (transactional checkout, ThriveCart product 7) both on disk, ready for WordPress deploy |
