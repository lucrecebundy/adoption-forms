...
| Qualifier quiz | `/quiz/` | 60-second eligibility quiz (Typeform embed) — page shell built | `qualifier-quiz-v1.md` + `quiz.html` | 🔵 Built — ready for WordPress deploy |
...
| Thank you / post-purchase | `/thank-you/` | Post-purchase landing page (noindex). Fires the Google Ads conversion event. All four ThriveCart products redirect here on successful payment. | `thank-you.html` (no separate spec doc — spec captured in master Section 10 entry 2026-09-18) | 🔵 Built — ready for WordPress deploy |
...
|---|---|---|---|
| 2026-09-18 | All 4 checkout HTML files + all 4 product spec docs (`dfy-minor-child-page-v1.md` v2.21 to v2.22, `dfy-adult-page-v1.md` v1.21 to v1.22, `diy-minor-child-page-v1.md` v1.15 to v1.16, `diy-adult-page-v1.md` v1.12 to v1.13) + **Master** v1.31 to v1.32 | **Third full four-product ThriveCart token regen in a single day.** Another round of ThriveCart admin changes regenerated all four tokens again. Product IDs, account name, and script src unchanged throughout. **Old to new tokens:** product 10 `UCMYFZ` to `5TNCQG` (eleventh regen overall); product 8 `E122QL` to `C2K6LW` (sixth); product 4 `49LJWX` to `ETAZ59` (sixth); product 7 `EIPBWZ` to `DCTFYF` (fifth). **Current live tokens: 10 = `5TNCQG`, 8 = `C2K6LW`, 4 = `ETAZ59`, 7 = `DCTFYF`.** Each swapped in its checkout HTML in 2 places, plus current-spec references in the three spec docs that carry them. Historical change-log entries naming old tokens left intact throughout. **Process note for future sessions:** three full regen rounds happened today because each ThriveCart admin save mints a new token and the repo was updated after each one. The cheaper pattern is to finish ALL ThriveCart admin work first, then capture all four embed codes once, update the repo once, and re-paste the four WordPress checkout pages once. Intermediate rounds only matter if the pages are re-pasted live in between — otherwise they are superseded before they ever reach production. | Claude / Tyler |
| 2026-09-18 | All 4 checkout HTML files + all 4 product spec docs (`dfy-minor-child-page-v1.md` v2.20 to v2.21, `dfy-adult-page-v1.md` v1.20 to v1.21, `diy-minor-child-page-v1.md` v1.14 to v1.15, `diy-adult-page-v1.md` v1.11 to v1.12) + **Master** v1.30 to v1.31 | **Second full four-product ThriveCart token regen today — caused by fixing the post-purchase redirect setup.** A test purchase revealed the buyer was being sent to ThriveCart hosted confirmation page (`adoptionssimplified.thrivecart.com/...../confirm/....`) rather than to `/thank-you/`. Diagnosis: the `/thank-you/` URL had been entered as a **product-delivery link** — which renders a "click here to access" link on ThriveCart own confirmation page — instead of as an **automatic success redirect**. That distinction matters for conversion tracking: the Google Ads conversion only fires when `/thank-you/` actually loads, so a link the buyer has to notice and click would undercount real sales. Tyler switched all four products to an automatic redirect, and saving each product regenerated its token again. **Old to new tokens:** product 10 `X801EE` to `UCMYFZ` (tenth regen overall); product 8 `DGBV8Y` to `E122QL` (fifth); product 4 `3TRK6C` to `49LJWX` (fifth); product 7 `I4RAVD` to `EIPBWZ` (fourth). **Current live tokens: 10 = `UCMYFZ`, 8 = `E122QL`, 4 = `49LJWX`, 7 = `EIPBWZ`.** Each swapped in its checkout HTML in 2 places, plus current-spec references in the three spec docs that carry them (`diy-minor-child-page-v1.md` has none). Historical change-log entries naming old tokens left intact throughout. **Open item:** Tyler should confirm what ThriveCart hosted confirmation page was previously delivering to customers (download links, product access) before relying solely on `/thank-you/`, which says "check your email". Also still pending: the real Google Ads gtag snippet replacing the placeholder comment in `thank-you.html`, and a fresh test purchase to confirm the automatic redirect now lands on `/thank-you/`. | Claude / Tyler |
| 2026-09-18 | All 4 checkout HTML files + all 4 product spec docs (`dfy-minor-child-page-v1.md` v2.18 → v2.20, `diy-minor-child-page-v1.md` v1.13 → v1.14, `dfy-adult-page-v1.md` v1.19 → v1.20, `diy-adult-page-v1.md` v1.10 → v1.11) + **Master** v1.29 → v1.30 | **ThriveCart token regen — `/thank-you/` redirect batch.** Cause: Tyler set the post-purchase success redirect to the new `/thank-you/` page in the ThriveCart admin, and saving each product regenerated its embeddable token — the same behavior seen after every prior admin-side change on this account (logo swap, pricing tweaks, etc.). Product IDs, account name, and script src all unchanged in every case; only the six-character token differs. **All four products regenerated.** **Old → new tokens:** product 10 (DFY minor child) `LB4DW4` → `1ZYMMF`, then a second time later the same session → **`X801EE`** (eighth and ninth regens overall for this product — Tyler made a further admin change after the first swap); product 4 (DIY minor child) `XTQG5V` → `9KZ1GB`, then a second time later the same session → **`3TRK6C`** (third and fourth regens overall); product 8 (DFY adult) `1PK8KJ` → `DGBV8Y` (fourth regen overall); product 7 (DIY adult) `U90O73` → `I4RAVD` (third regen overall). **Current live tokens are therefore: 10 = `X801EE`, 8 = `DGBV8Y`, 4 = `3TRK6C`, 7 = `I4RAVD`.** Each token swapped in its checkout HTML in 2 places (`data-thrivecart-embeddable` attr on the div + `id` on the script). **Spec-doc handling differs per doc:** `dfy-minor-child-page-v1.md` carries a current-spec ThriveCart snippet block, so its five current-spec references were updated; `dfy-adult-page-v1.md` has one current-spec token reference (the Section 8 checkout-page file/URL/token line), updated; `diy-adult-page-v1.md` has three (Section 8 snippet block ×2 + the embeddable-token line), updated; `diy-minor-child-page-v1.md` has no current-spec token reference at all — its only mentions are historical change-log entries, so no body edit was needed. In every doc, historical change-log entries naming the old tokens were deliberately left unchanged so the regen history stays accurate. **Worth noting for future sessions:** the redirect setting itself is server-side and does not require re-pasting the embed — but because *saving* a product regenerates its token, any ThriveCart admin change should be followed by a token check on the affected product. Products 10 and 4 each regenerating twice in this one session is the clearest illustration: each save produces a new token, so the check has to come after the *last* admin edit, not the first. **Labeling note:** one product-10 snippet was described as "DIY minor," but it carried `data-thrivecart-product="10"`, which is the **DFY** minor-child product — DIY minor child is product `4`. The product ID in the snippet was treated as authoritative, and the discrepancy was flagged; Tyler then confirmed product 4 had separately changed and supplied its own new snippet (`3TRK6C`), so both products were genuinely edited. **Takeaway for future regen rounds:** trust `data-thrivecart-product` in the snippet over the product nickname, and confirm which products were actually touched rather than inferring from the label. | Claude / Tyler |
| 2026-09-18 | **NEW `thank-you.html`** + **Master** v1.28 → v1.29 | **New `/thank-you/` post-purchase page built.** Self-contained page matching the site design system exactly — token block, base styles, buttons, nav CSS, section primitives, and footer CSS all copied from `quiz.html` (which matches `homepage.html`). Nav is the current post-2026-09-03 order (How It Works → Pricing → Free Guide → Quiz → FAQ → About → Blog → CTA); footer is the standard block with the full UPL disclaimer, all three legal-policy links, and the email-only Contact column. **Structure:** centered hero with generous vertical padding — coral circle + white checkmark (86px, `aria-hidden`, the one new visual element on the page), "PAYMENT CONFIRMED" kicker pill, headline "Thank you for your purchase.", subhead pointing to the inbox; then a "What happens next" 3-card section on `--color-cream-deep` using the homepage's italic coral `01 / 02 / 03` step numbering (check your email → review the materials → book your support call); then a muted italic reminder line covering the spam folder with `info@adoptionformsexpress.com` as a live `mailto:`. **Meta:** title "Thank You \| Adoption Forms Express", description, canonical `https://adoptionformsexpress.com/thank-you/`, OG + Twitter tags, and **`robots: noindex, nofollow`** (customer-only page). **Google Ads conversion tracking:** placeholder HTML comment block sits immediately before `</head>`, carrying the expected gtag snippet format (`AW-XXXXXXXXX` + `send_to` conversion label) for Tyler's Google Ads consultant to replace with the live snippet. No tracking code is active yet — the page fires nothing until that comment is replaced. **Voice/UPL:** "support call" framing used per the 2026-05-05 UPL language baseline; no legal-outcome promises; standard footer disclaimer present. **Outstanding wiring for Tyler (the page does nothing until these are done):** (a) create a top-level WP page at `/thank-you/` using the Blank Slate template and paste `thank-you.html`'s body content; (b) paste the real Google Ads snippet over the placeholder comment once the consultant sends the conversion ID + label; (c) set the post-purchase success redirect on **all four ThriveCart products** (10 / 8 / 4 / 7) to `https://adoptionformsexpress.com/thank-you/` — this is a ThriveCart admin setting, not a code change, and without it no buyer ever reaches the page and no conversion ever fires; (d) confirm `noindex` is not stripped by the WP SEO plugin. **Note:** no page-level spec doc was created — following the legal-pages precedent, the spec lives in this change-log entry. | Claude / Tyler |
| 2026-09-03 | `homepage.html`, `quiz.html`, 3 legal pages (`privacy-policy.html`, `terms-of-use.html`, `refund-policy.html`), `divi-header-snippet.html` + `homepage-redesign-v1.md` + **Master** | **Primary nav reordered site-wide.** New order: **How It Works → Pricing → Free Guide → Quiz → FAQ → About → Blog**, with the "See If You Qualify" CTA button pinned last (unchanged position). "Free Guide" and "Quiz" moved up ahead of FAQ/About/Blog to foreground the two top-of-funnel conversion paths (lead magnet + qualifier). Applied to all 6 files that carry the standard `.nav-links` nav — the 5 self-contained custom pages (each embeds its own nav) plus the Divi header snippet (the live global theme header). **The Divi header snippet also gained the "Quiz" text link it was previously missing** (it had only the CTA button), so it now matches the custom pages exactly. Gated product/checkout pages were untouched — they have no site nav (footer links only). Homepage uses `#anchor` hrefs for the on-page sections; the sub-pages + Divi snippet use `/#anchor` absolute hrefs — order is identical across all. Mobile behavior unchanged (nav collapses to just the CTA button below 860px, so the reorder is a desktop-only visual change). **Committed `39f4e28` and pushed to `origin/main`.** **Live-deploy reminder for Tyler:** because each custom page embeds its own nav copy, the new order won't appear live until the 5 pages are re-pasted in WordPress (Home + Quiz highest priority, then the 3 legal pages) AND the Divi header snippet is repasted into Theme Builder's global Header code module. (Longer-term option discussed: migrate the custom pages onto the single Divi global header/footer so future nav/footer edits are a one-place change — not yet actioned.) **Spec docs bumped:** homepage-redesign-v1.md v2.16 → v2.17; Master v1.27 → v1.28. | Claude / Tyler |
| 2026-09-02 | **NEW `quiz.html`** + `homepage.html` + 3 legal pages (`privacy-policy.html`, `terms-of-use.html`, `refund-policy.html`) + 8 gated product pages (4 sales + 4 checkouts) + both Divi snippets (`divi-header-snippet.html`, `divi-footer-snippet.html`) + `homepage-redesign-v1.md` + **Master** | **New `/quiz/` qualifier page built + all qualifier CTAs re-routed to it.** **(1) New page:** built self-contained `quiz.html` at `/quiz/`, matching the site design system exactly (nav + footer copied from `homepage.html`, shared design tokens, Fraunces/Manrope). Structure: centered hero (kicker "60-Second Quiz" + headline "See if your Nebraska stepparent adoption case qualifies for a forms-based path" + subhead), a centered Typeform embed (`data-tf-live="01HGCT4D29KZ4G50BG7556D9F4"` in a 900px-max container, 600px min-height, generous padding), and a "What happens next?" 3-card section (qualify → DIY $299 / DFY $499; don't qualify → full attorney representation via Bundy Law LLC; either way free / no purchase required). Meta: title "Nebraska Stepparent Adoption Qualifier Quiz | Adoption Forms Express", description, canonical `https://adoptionformsexpress.com/quiz/`, OG + Twitter tags, `robots: index, follow`. **(2) Nav:** added a "Quiz" link (`/quiz/`) immediately before the "See If You Qualify" CTA button on `homepage.html` + the 3 legal pages + `quiz.html` itself. `starter-guide.html` was intentionally skipped — it uses a minimal logo-only top-strip with no standard nav-links and no footer, so there is nowhere to add the link. **(3) CTA re-routing:** replaced every occurrence of the old Typeform qualifier URL `https://form.typeform.com/to/wnKZAY48` (which always carried `target="_blank" rel="noopener"`) with the internal path `/quiz/`, dropping the new-tab attributes since these are now on-site links. Scope — 14 files: homepage nav CTA + hero CTA + "Take the 60-Second Qualifier Quiz" + both pricing-card buttons ("Get DIY Forms — $299" / "Get Done-For-You — $499") + final-CTA button + footer "Qualifier Quiz" link; nav CTA on all 3 legal pages + `quiz.html`; the "Qualifier Quiz" footer link on all 3 legal pages, all 4 sales pages, all 4 checkout pages; and the shared CTA/footer links in **both Divi snippets** (these render the live WordPress header/footer, so they are what makes the routing change take effect site-wide). `grep` confirms zero remaining references to the old Typeform URL. **Note for Tyler:** the `/quiz/` page embeds a *different* Typeform (`01HGCT4D29KZ4G50BG7556D9F4`) than the old CTA URL (`wnKZAY48`) — confirm that ID is the intended published form. **WordPress deploy steps:** (a) create a top-level WP page at `/quiz/` using the Blank Slate template and paste `quiz.html`'s body content; (b) update the Divi header snippet + Divi footer snippet so the live shared nav CTA and footer "Qualifier Quiz" link point to `/quiz/`; (c) optionally 301 the old standalone Typeform link if it was linked anywhere off-site. **Spec docs bumped:** homepage-redesign-v1.md v2.15 → v2.16; Master v1.26 → v1.27. Section 2 Qualifier-quiz row flipped to 🔵 Built (URL `/quiz/`, `quiz.html`). | Claude / Tyler |
...
| 2026-05-12 | All 4 checkout HTML files + 4 spec docs + **Master** | **Four-product ThriveCart token regen + new purchase-agreement line on all four checkout pages.** **(1) Tokens:** Tyler made another round of admin-side adjustments across all four products. Product IDs unchanged. **Old → new tokens:** product 10 (DFY minor) `TAZ7KX` → `LB4DW4` (seventh regen for product 10 overall); product 8 (DFY adult) `OBAALE` → `1PK8KJ` (third for product 8); product 7 (DIY adult) `8XP52F` → `U90O73` (second for product 7); product 4 (DIY minor) `PC00K4` → `XTQG5V` (second for product 4). Tokens swapped in each checkout HTML (token in 2 places per file — div attr + script id) and in each spec doc via `replace_all`. **(2) Purchase-agreement line:** new line added uniformly to all 4 checkout HTML files immediately above the existing reassurance strip: *"By purchasing you agree to our refund policy."* with "refund policy" linked to `https://adoptionformsexpress.com/refund-policy/`. The link opens in a new tab (`target="_blank" rel="noopener"`) so it doesn't pull buyers away from completing their ThriveCart form. Reuses the existing `.refund-note` CSS class for visual consistency (no new CSS rules added). **Visual order on each checkout page is now:** embed → "By purchasing..." (agreement) → reassurance strip ("🔒 Secure ThriveCart checkout · Forms prepared by a licensed Nebraska adoption attorney") → "Wrong package?..." (existing refund-note). **Spec doc versions bumped:** dfy-minor-child-page-v1.md v2.17 → v2.18; dfy-adult-page-v1.md v1.18 → v1.19; diy-minor-child-page-v1.md v1.12 → v1.13; diy-adult-page-v1.md v1.9 → v1.10. Master v1.24 → v1.25. **Section 10 archive reminder:** Section 10 now has 12 inline entries — well past the ~5–7 threshold and the master is approaching the auto-load size warning. Queue an archive pass next session. | Claude / Tyler |
| 2026-05-12 | `dfy-minor-child-checkout.html`, `dfy-minor-child-page-v1.md`, **Master** | ThriveCart embed token regenerated **again** for **product 10 only** (DFY minor child) after Tyler made additional minor admin-side adjustments. Sixth regen overall for this product across the project. Product ID unchanged. Old token: `JB96LS` → new token: `TAZ7KX`. Token swapped in `dfy-minor-child-checkout.html` (2 places — div attr + script id) and across all token references in `dfy-minor-child-page-v1.md` via `replace_all`. Only product 10 affected this round — the other three products (4 / 7 / 8) keep their previously set tokens. Spec doc v2.16 → v2.17. Master v1.23 → v1.24. | Claude / Tyler |
...
| 2026-06-05 | All 13 deployed HTML files (`homepage.html` + 4 sales pages + 4 checkout pages + 3 legal pages + `divi-footer-snippet.html`) + `homepage-redesign-v1.md` v2.14 → v2.15 + **Master** v1.25 → v1.26 | **Mailing address removed site-wide.** Tyler asked to take the P.O. Box address off every page. **Scope of removal:** (a) the single-line `<li>P.O. Box 45873<br>Omaha, Nebraska 68145</li>` Contact bullet from all 9 deployed-HTML-page footers + the 3 legal-page footers + `divi-footer-snippet.html` (13 footer edits); (b) the two-line `<p>P.O. Box 45873</p><p>Omaha, Nebraska 68145</p>` pair from each of the 3 legal-page in-content `legal-contact-block` divs (privacy + terms + refund); (c) `streetAddress` and `postalCode` fields from the `homepage.html` `LegalService` JSON-LD `address` block — `addressLocality: Omaha`, `addressRegion: NE`, `addressCountry: US` retained (none required by schema.org, kept for local-SEO signal); (d) spec-doc consistency pass on `homepage-redesign-v1.md` Section 5.15 Column 4 Contact bullet + Section 8 schema example. **Visible result per surface:** every footer Contact column now reads as a single-line list with just the email; every legal-page contact block now reads "Adoption Forms Express → Email Address: info@…". **Sites of address that were already gone (no change needed):** the old `12020 Shamrock Plz, Ste 200, Omaha, NE 68154` line was the prior address — it was overwritten with the P.O. Box on 2026-05-12 and has been off the live site since then. **Reversibility:** the prior change-log entry (the 2026-05-12 four-row above) documents the full text of the removed address if it ever needs to be re-added. **Grep validation:** `grep -rn "45873\|68145\|P.O. Box" --include="*.html"` returns zero matches after this pass. Master v1.25 → v1.26; homepage spec v2.14 → v2.15. | Claude / Tyler |
...
|---|---|---|---|
| **Master** | `website-redesign-master.md` | v1.32 | 🟡 Active |
| Homepage | `homepage-redesign-v1.md` | v2.17 | 🔵 Built (`homepage.html`), ready for WordPress deploy |
| Qualifier quiz | `qualifier-quiz-v1.md` + `quiz.html` | — (page shell built) | 🔵 Built (`quiz.html`), ready for WordPress deploy |
| Thank you / post-purchase | `thank-you.html` | — (no spec doc) | 🔵 Built — ready for WordPress deploy. Spec captured in master Section 10 entry 2026-09-18. Google Ads snippet + ThriveCart success-redirect still to be wired. |
| Pricing | `pricing-page-v1.md` | — | ⚪ Not started |
| DFY minor child (sales + checkout) | `dfy-minor-child-page-v1.md` | v2.22 | 🔵 Built — `dfy-minor-child.html` (sales page) + `dfy-minor-child-checkout.html` (transactional checkout, ThriveCart product 10) both on disk, ready for WordPress deploy |
| DFY sales — adult | `dfy-adult-page-v1.md` | v1.22 | 🔵 Built — `dfy-adult.html` (sales page) + `dfy-adult-checkout.html` (transactional checkout, ThriveCart product 8) both on disk, ready for WordPress deploy |
| DIY minor child (sales + checkout) | `diy-minor-child-page-v1.md` | v1.16 | 🔵 Built — `diy-minor-child.html` (sales page) + `diy-minor-child-checkout.html` (transactional checkout, ThriveCart product 4) both on disk, ready for WordPress deploy |
| DIY adult (sales + checkout) | `diy-adult-page-v1.md` | v1.13 | 🔵 Built — `diy-adult.html` (sales page) + `diy-adult-checkout.html` (transactional checkout, ThriveCart product 7) both on disk, ready for WordPress deploy |
